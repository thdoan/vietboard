...
13. **Return to Game loses multiplayer state**: When clicking a high score in multiplayer, `localStorage['session']` was not overwritten (it's for SP), so "Return to Game" restored an old single-player session. **Fix**: Use dedicated `session_return` key for temporary storage while viewing high scores, and call `saveMultiplayerSession()` to snapshot MP state. `returnToGame()` then loads from `session_return`, clears it, and calls `updateTurnIndicator()` to restore button states.
